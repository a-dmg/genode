 ...
 */
struct Hw::Global_interrupt_controller : Genode::Mmio<0xf04>
{
 ...
 */
class Hw::Local_interrupt_controller : Genode::Mmio<0x14>
{
	private:
		...
		Distributor   &_distr;
		Iar::access_t  _last_iar { Iar::Irq_id::bits(_spurious_id) };
		unsigned const _max_irq  { _distr.max_irq() };

		bool _valid(unsigned const irq_id) const { return irq_id <= _max_irq; }

	public:

		Local_interrupt_controller(Distributor &distr);
		...
		void finish_request()
		{
			write<Eoir>(_last_iar);
			_last_iar = Iar::Irq_id::bits(_spurious_id);
		}
